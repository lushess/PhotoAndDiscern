...
void LCD_ShowFont12Char_FromBuf(uint16_t x, uint16_t y, uint8_t *Buf,u16 Word)
{
	uint8_t i, j, color, buf[32];
	uint8_t *pBuf=Buf;
	uint16_t asc,size=Word;
	uint32_t wordAddr = 0;
  while(size)
	{
    if(*pBuf != '\0')
    {
        ...
        {
            /* 将汉字编码转换成在FLASH中的地址 */
            asc = *pBuf - 0x81;     //高字节是表示分区，分区是从0x81到0xFE,所以转换成地址-0x80
            wordAddr = asc * 190; //每个分区一共有190个字
    
            asc = *(pBuf + 1); //低字节代表每个字在每个分区的位置，它是从0x40到0xFF
            if(asc < 0x7F)   //在0x7F位置有个空位，但是我们取模不留空，所以大于0x7F之后多减一
            {
                asc -= 0x40;
            }
