...
void OptionalFilter(vu8 *Image) //自适应选择性平滑滤波，其内有9个算子，能有效保留边沿
{
  ...
  vu8 *TmpImage=(vu8 *)malloc(240*320*sizeof(vu8)),*pTmpImage=TmpImage;
	if(TmpImage!=NULL)	UDEBUG("TmpImage内存分配成功,其地址为:%p\r\n",TmpImage);		
 	else UDEBUG("TmpImage内存分配失败\r\n");
...
void GrayGradientToMask(vu8 *Mask,vu8 *Gradient) //基于梯度图制作掩模，遮掩部分为255
{ 
  vu8 *pMask=Mask;
	vu16 i,j;
...
void Projective_H(vu16 *projection_x,vu8 *BinaryImage)
{
  ...
  vu16 i,j;
    ...
    for(i=1;i<=240;i++)
		...
		pprojection=projection_x; //指针指向投影数据开头
	}
