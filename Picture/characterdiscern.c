...
static void CharacterSegmentation_x(vu16 *xArray,vu8 *BinaryImage)//x数组每两个元素分别表示xleft,xright
{
  ...
  vu16 j;
	...
	if(ChangePointArray_V!=NULL)	UDEBUG("ChangePointArray_V内存分配成功,其地址为:%p\r\n",ChangePointArray_V);		
 	else UDEBUG("ChangePointArray_V内存分配失败\r\n");
  ...
  for(j=1;j<=240;j++)
  {
	...
	*pxArray=0;  //作为静态数组xArray特征点,迭代时判断以作停止
	...
	UDEBUG("ChangePointArray_V内存已释放,其地址为:%p\r\n",ChangePointArray_V);
...
static void CharacterSegmentation_y(vu16 *yArray,vu8 *BinaryImage)//y数组每两个元素分别表示yup,ylow
{
  ...
  vu16 i;
	...
	if(ChangePointArray_H!=NULL)	UDEBUG("ChangePointArray_V内存分配成功,其地址为:%p\r\n",ChangePointArray_H);		
 	else UDEBUG("ChangePointArray_H内存分配失败\r\n");
  ...
  for(i=1;i<=320;i++)
  {
	...
	*pyArray=0;
	...
	UDEBUG("ChangePointArray_H内存已释放,其地址为:%p\r\n",ChangePointArray_H);
...
static void SaveCharacterImage(vu8 *CharacterImage,vu8 *BinaryImage)//CharacterImage用于存储字符像素，其中用0x0f隔开
{
	...
	if(xArray!=NULL)	UDEBUG("xArray内存分配成功,其地址为:%p\r\n",xArray);		
 	else UDEBUG("xArray内存分配失败\r\n");
	...
	if(yArray!=NULL)	UDEBUG("yArray内存分配成功,其地址为:%p\r\n",yArray);		
 	else UDEBUG("yArray内存分配失败\r\n");
...
//			if(deltax<Charater_xmaxForNormalization) continue;//舍去过小字符
  ...
  *pCharacterImage=0xf0;       //输入数组结束特征值
	...
	UDEBUG("xArray内存已释放,其地址为:%p\r\n",xArray);
	Memfree((void *)xArray);
	UDEBUG("yArray内存已释放,其地址为:%p\r\n",yArray);
	Memfree((void *)yArray);
...
void CharacterCompareFromExflash(vu8 *BinaryImage,vu16 *Character_GB2312) //存储读取到的字符的编码
{
  ...
  vu16 ConfindeceFlag,encode;	
	...
	vu16 *Confidence=(vu16 *)Memalloc(CharacterMatchBuf*sizeof(vu16));  //记录外部flash粗比对缓冲区字模置信度，其标志数和缓冲区一致，不能打乱
	vu16 *TmpCharacter_GB2312=Character_GB2312,*pConfidence=Confidence;
	if(Confidence!=NULL)	UDEBUG("Confidence内存分配成功,其地址为:%p\r\n",Confidence);		
 	else UDEBUG("Confidence内存分配失败\r\n");

	u32 *CharacterMatch=(u32 *)Memalloc(CharacterMatchBuf*sizeof(u32));;  //存储从外部flash读取的粗比对成功的字模地址
	u32 *pCharacterMatch=CharacterMatch;
	if(CharacterMatch!=NULL)	UDEBUG("CharacterMatch内存分配成功,其地址为:%p\r\n",CharacterMatch);		
 	else UDEBUG("CharacterMatch内存分配失败\r\n");

	vu8 *CharacterMatrix=(vu8 *)Memalloc(32*CharacterNumToCapture*sizeof(vu8));    //字模为16*16，即32*8位，最多存储20个字
	if(CharacterMatrix!=NULL)	UDEBUG("CharacterMatrix内存分配成功,其地址为:%p\r\n",CharacterMatrix);		
 	else UDEBUG("CharacterMatrix内存分配失败\r\n");
...
//		pCharacterMatch=CharacterMatch;
	...
	UDEBUG("CharacterMatrix内存已释放,其地址为:%p\r\n",CharacterMatrix);
  Memfree((void *)CharacterMatrix);
	UDEBUG("Confidence内存已释放,其地址为:%p\r\n",Confidence);
	Memfree((void *)Confidence);
	UDEBUG("CharacterMatch内存已释放,其地址为:%p\r\n",CharacterMatch);
