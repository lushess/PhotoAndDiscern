...
void Touch_lvgl_read(lv_indev_drv_t * indev_drv, lv_indev_data_t * data)
{
    ...
    if(Touch_debouncing() == TOUCH_IS_PRESSED)
    {
			if(!TOUCH_ReadXY(&TouchData.x, &TouchData.y)) //触摸接收到
			{
				ret=1;
			}
			if(ret==1)
			{
					...
					last_x = TouchData.lcdx;
					last_y = TouchData.lcdy;
