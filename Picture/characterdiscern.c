...
void CharacterCompareFromExflash(vu8 *BinaryImage,vu16 *Character_GB2312) //存储读取到的字符的编码
{
  ...
  vu16 ConfindeceFlag,encode;	
	...
	vu16 *Confidence=(vu16 *)malloc(CharacterMatchBuf*sizeof(vu16));  //记录外部flash粗比对缓冲区字模置信度，其标志数和缓冲区一致，不能打乱
	...
	u32 *CharacterMatch=(u32 *)malloc(CharacterMatchBuf*sizeof(u32));;  //存储从外部flash读取的粗比对成功的字模地址
	...
	vu8 *CharacterMatrix=(vu8 *)malloc(32*CharacterNumToCapture*sizeof(vu8));    //字模为16*16，即32*8位，最多存储20个字
